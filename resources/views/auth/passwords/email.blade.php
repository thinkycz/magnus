            ...
            <form method="POST" action="{{ route('password.email') }}" class="flex items-stretch bg-white rounded-lg shadow-lg overflow-hidden border-t-4 border-indigo-500 sm:border-0">
                ...
                <div class="flex hidden overflow-hidden relative sm:block w-5/12 md:w-6/12 bg-gray-600 text-gray-300 py-4 bg-cover bg-center" style="background-image: url('{{ asset('img/prague.jpg') }}')">
                    <div class="flex-1 absolute bottom-0 text-white p-10">
                        <h3 class="text-2xl font-bold inline-block">Đặt lại mật khẩu</h3>
                ...
                <div class="flex-1 p-6 sm:p-10 sm:py-12">
                    <h3 class="text-xl text-gray-700 font-bold mb-6">
                        Địa chỉ Email <span class="text-gray-400 font-light">của bạn</span></h3>

                    <input id="email" type="email" class="px-3 w-full py-2 bg-gray-200 border border-gray-200 rounded focus:border-gray-400 focus:outline-none focus:bg-white mb-4 {{ $errors->has('email') ? ' border-red' : '' }}" name="email" value="{{ old('email') }}" required placeholder="Địa chỉ Email">
                    ...
                    <div class="flex flex-wrap items-center">
                        <div class="w-full sm:flex-1">
                            <input type="submit" value="Gửi email đặt lại mật khẩu" class="w-full sm:w-auto bg-green-500 text-green-100 px-6 py-3 rounded-full hover:bg-green-600 focus:outline-none cursor-pointer">
                        </div>
                    </div>
